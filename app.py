...
# ---------- Configuration ----------
JSONCLIP_API_KEY = os.environ.get("JSONCLIP_API_KEY", "")
...
<body>
  <div class="container py-5">
    ...
    <h2>⚡ Quick Start</h2>
    <pre class="bg-dark text-light p-3 rounded"><code>https://your-domain/generate?image=https://example.com/img.jpg&width=720&height=1280&fps=30&background_color=%23000000&duration=2000&audio=https://example.com/audio.mp3</code></pre>
    ...
    <h2>🧪 Code Examples</h2>
    <h3>cURL</h3>
    <pre class="bg-dark text-light p-3"><code>curl "https://your-domain/generate?image=https://example.com/img.jpg&width=720&height=1280&fps=30&duration=2000"</code></pre>

    <h3>Python</h3>
    <pre class="bg-dark text-light p-3"><code>import requests
url = "https://your-domain/generate"
params = {"image": "https://example.com/img.jpg", "width": 720, "height": 1280, "fps": 30}
resp = requests.get(url, params=params)
print(resp.json())
# {
#   "success": true,
#   "video": "https://your-domain/video/uuid-here",
#   "DEV": "@R4_QN",
#   "CHANNEL": "@R4_QX"
# }</code></pre>

    <h3>JavaScript (fetch)</h3>
    <pre class="bg-dark text-light p-3"><code>fetch('https://your-domain/generate?image=https://example.com/img.jpg&width=720&height=1280')
  .then(r => r.json())
  .then(data => console.log(data.video));</code></pre>

    <h3>Browser</h3>
    <p>Simply paste this URL in your browser's address bar:</p>
    <pre class="bg-dark text-light p-3"><code>https://your-domain/generate?image=https://example.com/img.jpg&width=720&height=1280</code></pre>
...
def documentation():
    ...
    return render_template_string(DOCS_HTML, params=PARAM_DEFS, dev=DEV, channel=CHANNEL)
...
def generate_video():
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": "XK VIDEO GENERATOR request timed out.",
            "DEV": DEV,
            "CHANNEL": CHANNEL
        }), 504
        ...
        return jsonify({
            "success": False,
            "error": f"XK VIDEO GENERATOR request timed out. {str(e)}",
            "DEV": DEV,
            "CHANNEL": CHANNEL
        }), 502
    except ValueError:
        return jsonify({
            "success": False,
            "error": "Invalid JSON response from XK VIDEO GENERATOR.",
            "DEV": DEV,
            "CHANNEL": CHANNEL
        }), 502
    ...
    if not real_video_url:
        return jsonify({
            "success": False,
            "error": "Video URL not found in XK VIDEO GENERATOR response .",
            "DEV": DEV,
            "CHANNEL": CHANNEL
        }), 502
